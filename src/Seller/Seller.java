...
public class Seller {
    ...
    DatabaseConnection dc = new DatabaseConnection("com.mysql.cj.jdbc.Driver", "jdbc:mysql://localhost:3306/market", "root", "");
    ...
    public String getBill(String q, String pro) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                JOptionPane.showMessageDialog(null, "Error while Connecting To The Database", "Database Connection Error", JOptionPane.ERROR_MESSAGE);
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
    ...
    public boolean addToOrder(String category, String product, int quantity, double bill, boolean orderIsFinished) {
        ...
        try {
            ...
            ResultSet resultSet = dc.executeQuery("select quantity,bill from orders where order_id='" + order_id + "' and product_name='" + product + "'");
            if (resultSet.next()) {
                int q = resultSet.getInt("quantity") + quantity;
                double b = resultSet.getDouble("bill") +bill;

                dc.excuteUpdate("update orders set quantity='" + q + "' , bill='" + b + "'where order_id='" + order_id + "' and product_name='" + product + "'");
            } else {
                dc.excuteUpdate("insert into orders (order_id,product_name,product_category,quantity,bill,date) values ('" + order_id + "','" + product + "','" + category + "','" + quantity + "','" + bill + "','" + LocalDateTime.now() + "')");
            }
            //update quantity
    ...
    double updateQuantity(int qdiff, String pname, String order_id) {
        ...
        ResultSet rs2 = dc.executeQuery("select quantity from orders where order_id='" + order_id + "' and product_name='" + pname + "'");
                int oquantity=0;
            try {
                if(rs2.next())
        ...
        if (qdiff == 0) {
            ...
            if (squantity-qdiff >= 0) {
                ...
                dc.excuteUpdate("update orders set quantity ='" + oquantity + "' , bill='" + bill + "' where product_name='" + pname + "' and order_id='" + order_id + "'");
                
                return Double.parseDouble(bill);
            } else {
                JOptionPane.showMessageDialog(null, "Current quantity For this Product Won't Satisfy What You Need ", "Quantity Error", JOptionPane.ERROR_MESSAGE);
                return 0;
            }
        } else {
            squantity += qdiff;
...
void deleteTransaction(String order_id,String pname,String quantity)
    {
